...
export default function App() {
  ...
  const addProject = (e) => {
    e.preventDefault();
    if (!projectForm.company || !projectForm.name) return;
    const rec = {
      id: crypto.randomUUID(),
      company: projectForm.company,
      contactPerson: projectForm.contactPerson,
    ...
    setImportMessage("Proje eklendi.");
  };
    ...
    if (mode === "companies") {
      const mapped = rows
        .map((r) => ({
          ...
          description: firstNonEmpty(r.description, r.aciklama, r.açıklama, r.detail, r.details),
          createdAt: new Date().toISOString(),
        }))
      ...
      <main style={styles.main}>
        ...
        {view === "projects" && (
          <section style={styles.grid2}>
            <Panel title="Projects Listesi">
              {projects.map((p) => (
                <Card key={p.id}>
                  ...
                  {p.description && <div style={styles.projectDescriptionPreview}>{p.description}</div>}
                  <div style={styles.cardActions}>
            ...
            <Panel title="Yeni Project">
              <form onSubmit={addProject} style={styles.form}>
                <InputField label="Company" value={projectForm.company} onChange={(v) => setProjectForm({ ...projectForm, company: v })} />
                <InputField label="Contact Person" value={projectForm.contactPerson} onChange={(v) => setProjectForm({ ...projectForm, contactPerson: v })} />
                <InputField label="Name" value={projectForm.name} onChange={(v) => setProjectForm({ ...projectForm, name: v })} />
                <InputField label="Status" value={projectForm.status} onChange={(v) => setProjectForm({ ...projectForm, status: v })} />
                <InputField label="Start Date" value={projectForm.startDate} onChange={(v) => setProjectForm({ ...projectForm, startDate: v })} />
                <InputField label="Due Date" value={projectForm.dueDate} onChange={(v) => setProjectForm({ ...projectForm, dueDate: v })} />
                <InputField label="Est. Revenue" value={projectForm.estRevenue} onChange={(v) => setProjectForm({ ...projectForm, estRevenue: v })} />
                <InputField label="Owner" value={projectForm.owner} onChange={(v) => setProjectForm({ ...projectForm, owner: v })} />
                <label style={styles.field}>
                  ...
                  <textarea
                    ...
                    placeholder="Proje kapsamı, teslim notları, müşteri beklentileri, dosya bilgileri, terminoloji notları gibi detayları buraya yazabilirsin."
                  />
                </label>
                <button style={styles.primaryBtn}>Project Ekle</button>
              </form>
            </Panel>
          </section>
        )}
            ...
            <Panel title="Kayıt Detayı">
              {activeRecord ? (
                <>
                  ...
                  {activeDetailType === "project" && activeRecord.description && (
                    <div style={styles.detailBox}>
                      <div style={styles.detailBoxTitle}>Proje detayı</div>
                      <div style={styles.detailBoxBody}>{activeRecord.description}</div>
                    </div>
                  )}
                  <div style={styles.cardActions}>
...
const styles = {
  ...
  textareaLg: { width: "100%", minHeight: 180, padding: "12px 14px", borderRadius: 12, border: "1px solid #e7d7ba", outline: "none", fontSize: 14, background: "#fffdf8", color: "#2B2B2B", resize: "vertical", lineHeight: 1.6 },
  ...
  projectDescriptionPreview: { marginTop: 10, color: "#5f4a1f", fontSize: 13, lineHeight: 1.6, display: "-webkit-box", WebkitLineClamp: 3, WebkitBoxOrient: "vertical", overflow: "hidden" },
  detailBox: { marginTop: 14, padding: 14, borderRadius: 14, background: "#fffaf0", border: "1px solid #f1e4c8" },
  detailBoxTitle: { fontSize: 12, fontWeight: 800, letterSpacing: 0.4, textTransform: "uppercase", color: "#8A6322", marginBottom: 8 },
  detailBoxBody: { whiteSpace: "pre-wrap", lineHeight: 1.7, color: "#2B2B2B", fontSize: 14 },
};
