...
The terminal commands in this guide are written for a Linux/macOS-style shell. If you are using Windows, one of the following options is recommended:

- **WSL (Windows Subsystem for Linux):** The most reliable option. You can install a Linux distribution such as Ubuntu and run the commands exactly as shown in this guide.
- **Git Bash:** Usually sufficient for basic `git`, `npm`, `node`, and file system commands.

### Quick recommendations for Windows users

1. Install Node.js using the official installer: [nodejs.org](https://nodejs.org/)
2. Install Git and, if possible, use **Git Bash**: [git-scm.com](https://git-scm.com/)
3. For a more stable development setup, prefer **WSL + Ubuntu**.
4. If you use PowerShell, note that some commands are different; for example, `rm -rf` becomes `Remove-Item -Recurse -Force`.

> **Recommendation:** This project can run on Windows as well, but WSL or Git Bash will make it much easier to follow the commands in this guide exactly as written.
